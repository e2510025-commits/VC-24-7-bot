# VC-24-7 Bot Commands

## Japanese (日本語)

- /auth-panel post channel:<#channel> - 認証パネルを設置（管理者）
- /auth-admin show - 現在の認証ロールを表示（管理者）
- /auth-admin set-role role:<role> - 認証ロールを設定（管理者）
...
- /auth-panel post channel:<#channel> - Post verification panel (admin)
- /auth-admin show - Show verification role (admin)
- /auth-admin set-role role:<role> - Set verification role (admin)
